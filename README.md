...
2. Run `node copy-custom.js`. It will copy all files from `site/custom` to `src/files/custom/Espo/Modules/InlineListEdit` and apply needed modifications to files.
3. Remove files from `site/custom`.
4. Run `npm run sync`. It will copy files from the repository to Espo build (`site/custom//Espo/Modules/InlineListEdit` dir).
...
1. Create a file `src/files/custom/Espo/Modules/InlineListEdit/composer.json` with your dependencies. You can change dir to this directory and add composer dependencies using *composer require*.
2. Once you run `node build --all` or `node build --composer-install`, composer dependencies will be automatically installed.
3. Create a file `src/files/custom/Espo/Modules/InlineListEdit/Resources/autoload.json`.
...
{
    "psr-4": {
        "LibraryNamespace\\": "custom/Espo/Modules/InlineListEdit/vendor/<vendor-name>/<library-name>/path/to/src"
    }
}
```
...
```
    excludePaths:
        - src/files/custom/Espo/Modules/InlineListEdit/vendor
    scanDirectories:
        - site/custom/Espo/Modules/InlineListEdit/vendor
```
...
```
vendor/bin/phpunit tests/unit/Espo/Modules/InlineListEdit
```
...
Unit tests should be placed in `tests/unit/Espo/Modules/InlineListEdit` directory and be in `tests\unit\Espo\Modules\InlineListEdit`
namespace.
...
```
(npm run sync; cd site; vendor/bin/phpunit tests/integration/Espo/Modules/InlineListEdit)
```
...
Integration tests should be placed in `tests/integration/Espo/Modules/InlineListEdit` directory
and be in `tests\integration\Espo\Modules\InlineListEdit` namespace.
...
* `site/custom/`
* `site/client/custom/`
* `site/tests/unit/Espo/Modules/InlineListEdit`
* `site/tests/integration/Espo/Modules/InlineListEdit`
...
2. Set *bundled* and *jsTranspiled* to true in `src/files/custom/Espo/Modules/InlineListEdit/Resources/module.json`.
3. Add `src/files/custom/Espo/Modules/InlineListEdit/Resources/metadata/app/client.json`
    ```json
    {
        "scriptList": [
            "__APPEND__",
            "client/custom/modules/inline-list-edit/lib/init.js"
        ]
    }
    ```
...
Add the library module path to `src/files/custom/Espo/Modules/InlineListEdit/Resources/metadata/app/jsLibs.json`

```json
{
    "some-lib": {
        "path": "client/custom/modules/inline-list-edit/lib/some-lib.js"
    }
}
```
